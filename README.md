...
기존의 '정적(Static)' 체크리스트는 사용자가 인지하고 있는 아이템의 관리에 그칠 뿐, 사용자가 인지하지 못해 <b>'정말 빠뜨린 짐(Unknown Unknowns)'</b>을 제안하지 못하는 근본적인 한계가 있습니다.

본 프로젝트의 목표는, 이 한계를 <b>'데이터 기반 추천'</b>으로 해결하는 지능형 엔진을 구현하는 것입니다.

이 엔진은 두 개의 핵심 데이터 벡터(Vector)를 기반으로 동작합니다.

1.  **[집단 지성] (Collective Intelligence):** 공유된 수많은 패킹 리스트에서 도출한 통계적, 경험적 데이터.
2.  **[개인화] (Personalization):** 사용자가 사전에 등록한 '내 옷장(My Closet)'의 아이템 자산(Asset) 데이터.

엔진의 **핵심 독창성**은, 단순히 "출장에 셔츠 챙기세요"라는 1차원적 제안이 아니라, `[집단 지성]`이 제안한 아이템을 `[개인화]` 데이터와 대조하여, <b>"현재 '내 옷장'에 없는 필수 아이템: 정장 셔츠"</b>와 같이 '격차 분석(Gap Analysis)'에 기반한 실용적인 추천을 생성하는 데 있습니다.

최종 목표는 이 두 데이터 벡터를 융합하는 지능형 추천 로직을 프레임워크 없이 순수 Java로 구현하는 것입니다.
...
* **📊 패킹 분석 리포트**: '내 옷장' 활용도 분석 및 여행에 사용된 '미등록 아이템'의 옷장 동기화 기능을 제공합니다.
...
<small>[기능 설명] `[SMART]` 추천 카테고리 제안 (y/n) - "반팔" 입력 시 "의류"를 자동 추천합니다.</small>
...
<small>[기능 설명] `[CHECK]` 저장 직전, AI가 추천했으나 빠뜨린 아이템을 마지막으로 점검합니다.</small>
...
[기능 설명] 인메모리 DB를 활용한 핵심 로직(Repository, Service) 테스트를 모두 통과한 화면입니다.
...
이 프로젝트는 Spring 같은 프레임워크의 핵심 원리(DI, 계층 분리)를 직접 구현하며 객체지향 원칙을 적용하는 것을 목표로 구축되었습니다.
...
* 이 설계 덕분에, **메인 코드**에서는 `JDBC` 구현체를 주입하고, **테스트 코드**에서는 `UserService` 코드 수정 없이 `인메모리 DB`용 구현체를 주입하여 테스트 환경의 **격리가 가능**했습니다.

### 3. 유지보수성을 위한 클린 코드
* **`AppConfig` (설정 중앙화):** `DB_URL`, `MAX_AI_RECOMMENDATIONS` 등 바뀔 수 있는 값들을 `AppConfig`에서 중앙 관리하여 <b>'매직 넘버'</b>를 제거하고, 향후 설정 변경이 필요할 때 단 한 곳만 수정하면 되도록 설계했습니다.
...
* **해결:**
    ...
    * JSON 내부의 `category` 값을 읽어 `ClothingItem`인지 `ElectronicItem`인지 동적으로 판단하여, **DB에 저장된 JSON에서 객체의 다형성을 안정적으로 복원**해냈습니다.
...
* **해결:**
    * 테스트 실행 시마다 메모리에서 생성되고 종료 시 깨끗하게 사라지는 <b>`인메모리 DB(jdbc:sqlite::memory:)`</b>를 사용하도록 테스트 환경을 **성공적으로 격리**했습니다.
    * 이는 "테스트는 몇 번을 돌려도 항상 같은 결과가 나와야 한다"는 테스트의 멱등성을 보장하며, 향후 리팩토링 시 코드가 망가지는 '퇴보(Regression)'를 효과적으로 방지합니다.
