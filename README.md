...
| `System.console()`을 활용한<br>비밀번호 마스킹 및 보안 인증 | '스마트 템플릿'과<br>'AI 꿀팁 추천'의 핵심 로직 | `is_shared=true`로 설정된<br>AI 학습 데이터셋 조회 기능 |
...
* **해결:**
    * 테스트 실행 시마다 메모리에서 생성되고 종료 시 깨끗하게 사라지는 **`인메모리 DB(jdbc:sqlite::memory:)`**를 사용하도록 테스트 환경을 **완벽히 격리**했습니다.
